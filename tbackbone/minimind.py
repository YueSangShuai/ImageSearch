...
class MiniMindLM(nn.Module):
    ...
    def forward(self,
        ...
        encoded = self.norm(h)
        output_embedding = torch.mean(encoded, dim=1, keepdim=False)
...
class L1(MiniMindLM): # 25.197M
    def __init__(self, embedding_dim=512, max_seq_length=1024, vocab_size=6400, cls_token_id=1,
        ...
        super().__init__(vocab_size=6400, n_layers=n_layers, n_heads=n_heads, dim=dim, n_kv_heads=n_kv_heads, 
            dropout=dropout, hidden_dim=hidden_dim, max_seq_len=max_seq_length, flash_attn=flash_attn)
    ...
    def forward(self, input_ids, past_key_values=None, use_cache=False):
        # padding a cls token at the last for each sequence in batch
        if use_cache:
...
class L4(L1): # 14.585M
    def __init__(self, embedding_dim=512, max_seq_length=1024, vocab_size=6400, cls_token_id=1):
        super().__init__(embedding_dim=embedding_dim, max_seq_length=max_seq_length, vocab_size=vocab_size, cls_token_id=cls_token_id, 
            n_layers=8, n_heads=8, dim=384, n_kv_heads=2, dropout=0.1, hidden_dim=1024, flash_attn=False)
