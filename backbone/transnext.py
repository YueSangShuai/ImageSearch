...
class TransNeXt(nn.Module):
    ...
    def __init__(self, img_size=224, pretrain_size=None, window_size=[3, 3, 3, None],
        ...
        self.feature_info = FeatureInfo([*embed_dims, num_classes],[4, 8, 16, 32,1024])
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            x = norm(x)
            y.append(x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous())
            if i != self.num_stages - 1:
                x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()

        return x.mean(dim=1), y
...
class micro(TransNeXt):
    def __init__(self, out_features=512, patch_size=4, **kwargs):
        super().__init__(num_classes=out_features, window_size=[3, 3, 3, None],
                      embed_dims=[48, 96, 192, 384], num_heads=[2, 4, 8, 16],
                      patch_size=patch_size, mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
                      norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 15, 2], sr_ratios=[8, 4, 2, 1],
                      **kwargs)

class micro112(micro):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(out_features=out_features, img_size=112, patch_size=2, **kwargs)

class tiny(TransNeXt):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(num_classes=out_features, window_size=[3, 3, 3, None],
                      patch_size=4, embed_dims=[72, 144, 288, 576], num_heads=[3, 6, 12, 24],
                      mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
