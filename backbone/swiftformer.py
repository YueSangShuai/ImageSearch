...
from .utils import DropPath, trunc_normal_, to_2tuple
import einops
...
class SwiftFormer_XS(SwiftFormer):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(layers=[48, 56, 112, 220],
            embed_dims=[3, 3, 6, 4],
            downsamples=[True, True, True, True],
            vit_num=1, num_classes=out_features,
            **kwargs)

class SwiftFormer_L1(SwiftFormer):
    def __init__(self, out_features=512, **kwargs):
            super().__init__(layers=[48, 96, 192, 384],
            embed_dims=[4, 3, 10, 5],
            downsamples=[True, True, True, True],
            vit_num=1, num_classes=out_features,
            **kwargs)
...
class SwiftFormer_L3(SwiftFormer):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(layers=[64, 128, 320, 512],
            embed_dims=[4, 4, 12, 6],
            downsamples=[True, True, True, True],
            vit_num=1, num_classes=out_features,
            **kwargs)
