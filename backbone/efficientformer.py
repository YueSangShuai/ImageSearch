...
from .utils import DropPath, trunc_normal_, to_2tuple, FeatureInfo
...
class EfficientFormer(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None,
        ...
        c = 2
        self.feature_info = FeatureInfo(embed_dims[:4]+[num_classes],[c,c*2,c*4,c*8,1024])
    ...
    def forward(self, x, return_featuremaps=False):
        ...
        for idx, block in enumerate(self.network):
            ...
            if idx in self.out_indices:
                norm_layer = getattr(self, f'norm{idx}')
                x_out = norm_layer(x.permute(0, 2, 3, 1).contiguous()).permute(0, 3, 1, 2).contiguous()
                outs.append(x_out)
        x = self.norm(x)
...
if __name__=="__main__":
    from . import utils
    model = utils.test_model(l3, [3,224,224])
