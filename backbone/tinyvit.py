...
class TinyViTBlock(nn.Module):
    ...
    def forward(self, x):
        H, W = self.input_resolution
        B, L, C = x.shape
        assert L == H * W, f"input {x.shape} feature has wrong size{L}!={H}*{W}"
        res_x = x
...
if __name__=="__main__":
    from . import utils
    m = utils.test_model(TinyVit_5m384, (3,384,384))
    torch.save(m.state_dict(), "TinyVit_5m384.pth")