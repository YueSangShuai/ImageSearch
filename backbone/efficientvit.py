...
from .utils import trunc_normal_, make_divisible, FeatureInfo, SqueezeExcite
...
class CascadedGroupAttention(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,C,H,W)
        B, C, H, W = x.shape
        dim = int(self.key_dim)
        #dim = self.key_dim*H*W
        ...
        for i, qkv in enumerate(self.qkvs):
            ...
            feat = qkv(feat)
            # print(i, feat.shape) #[32, 96, 7, 7]
            # q, k, v = feat.view(B, -1, H, W).split([self.key_dim, self.key_dim, self.d], dim=1) # B, C/h, H, W
            # print(i, q.shape, k.shape, v.shape) #[32, 16, 7, 7][32, 16, 7, 7][32, 64, 7, 7]
            # q = self.dws[i](q)
            # q, k, v = q.flatten(2), k.flatten(2), v.flatten(2) # B, C/h, N
            # print(i, q.shape, k.shape, v.shape) #[32, 16, 49][32, 16, 49][32, 64, 49]
            q = self.dws[i](feat[:,:dim]).flatten(2)
            k = feat[:,dim:dim*2].flatten(2)
            v = feat[:,dim*2:].flatten(2)
            attn = (
...
class EfficientViT(torch.nn.Module):
    def __init__(self, img_size=224,
        ...
        self.feature_info  = FeatureInfo(embed_dim+[num_classes], [16, 32, 64, 1024])
    ...
    def forward(self, x, return_featuremaps=False):
        B,N,H,W = x.shape
        if not (H==224 and W==224): x = F.interpolate(x, 224)
...
class M0(EfficientViT):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(**EfficientViT_m0, num_classes=out_features, **kwargs)

class M1(EfficientViT):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(**EfficientViT_m1, num_classes=out_features, **kwargs)

class M2(EfficientViT):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(**EfficientViT_m2, num_classes=out_features, **kwargs)

class M3(EfficientViT):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(**EfficientViT_m3, num_classes=out_features, **kwargs)       

class M4(EfficientViT):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(**EfficientViT_m4, num_classes=out_features, **kwargs)

class M5(EfficientViT):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(**EfficientViT_m5, num_classes=out_features, **kwargs)
