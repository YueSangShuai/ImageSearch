...
class HSViTModel(nn.Module):
    ...
    def forward(self, pixel_values):
        ...
        # print(conv2d_embeddings.shape) # torch.Size([3, 512, 64])
        if self.attn_num > 0:  # if enable self-attention
            ...
            for conv2d_embeddings, attn_encoder in zip(split_conv2d_embeddings, self.attn_encoders):
                ...
                # print("hidden_state:", hidden_state.shape) # hidden_state: torch.Size([3, 33, 64])
...
class HSViTForImageClassification(nn.Module):
    def __init__(
        ...
        self.classifier = ClassificationHead(attn_embed_dim, num_classes) if num_classes>0 else nn.Identity()
...
class Base(HSViTForImageClassification):
    # 27M/4 params
    def __init__(self, out_features=256, num_channels=3,
            image_size=224,
            conv_kernel_nums=[32, 64, 128, 256, 512],
        ...
        super().__init__(num_classes=out_features,                          
            ...
            conv_kernel_nums=conv_kernel_nums,
...
class Base128(Base):
    def __init__(self, out_features=256):
        super().__init__(out_features=out_features, image_size=128)

class Tiny(Base):
    # 6.8M/4 params
    def __init__(self, out_features=256):
        super().__init__(out_features=out_features,
            conv_kernel_nums=[32, 64, 128, 256],
...
if __name__ == '__main__':
    utils.test_model(Base, [3,224,224], 100)
