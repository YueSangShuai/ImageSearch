...
from .utils import DropPath_new as DropPath, trunc_normal_, drop_path
...
class EfficientVSSM(VSSM):
    ...
    def forward(self, x: torch.Tensor, return_featuremaps=False):
        x = self.patch_embed(x) 
        y = []
...
class Base(EfficientVSSM):
    def __init__(self, out_features=512, patch_size=4, 
            in_chans=3, 
            depths=[2, 2, 9, 2], 
            dims=[96, 192, 384, 768], 
            d_state=16, 
