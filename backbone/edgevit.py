...
class EdgeViT(nn.Module):
    def __init__(self, channels, blocks, heads, r=[4, 2, 2, 1], out_features=1000, distillation=False):
        super().__init__()
...
def EdgeViT_XXS(pretrained=False, **wargs):
    model = EdgeViT(**edgevit_configs['XXS'], **wargs)
...
def EdgeViT_XS(pretrained=False, **wargs):
    model = EdgeViT(**edgevit_configs['XS'], **wargs)
...
def EdgeViT_S(pretrained=False, **wargs):
    model = EdgeViT(**edgevit_configs['S'], **wargs)
...
if __name__=="__main__":
    ...
    net = EdgeViT_XXS(out_features=2)
    net.eval()
    y = net(x)
    print(net)
    print(y.shape)
    torch.save(net.state_dict(), "EdgeViT_XXS.pth")
