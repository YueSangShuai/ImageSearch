...
from .utils import to_2tuple, trunc_normal_, DropPath_new as DropPath
...
class Tiny(Dilateformer):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(num_classes=out_features, depths=[2, 2, 6, 2], embed_dim=72, num_heads=[ 3, 6, 12, 24 ], **kwargs)


class Small(Dilateformer):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(num_classes=out_features, depths=[3, 5, 8, 3], embed_dim=72, num_heads=[ 3, 6, 12, 24 ],  **kwargs)


class Base(Dilateformer):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(num_classes=out_features, depths=[4, 8, 10, 3], embed_dim=96, num_heads=[ 3, 6, 12, 24 ],  **kwargs)
