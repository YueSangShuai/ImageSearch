...
#############################################################################################  
        ...
        displacement = (q_grid.reshape(N * self.n_groups, H * W, 2).unsqueeze(2) - pos_k.reshape(N * self.n_groups, -1, 2).unsqueeze(1)).mul(0.5)
        attn_bias = F.grid_sample(
                    input=rearrange(rpe_bias, 'b (g c) h w -> (b g) c h w', c=self.n_group_heads, g=self.n_groups),
                    grid=displacement[..., (1, 0)],
                    mode='bilinear', align_corners=True) # B * g, h_g, HW, Ns
        attn_bias = attn_bias.reshape(N * self.num_heads, H * W, -1)
        attn = attn + attn_bias
        ### 
...
def debi_tiny(out_features, **wargs):
    model = DeBiFormer(
            depth=[1, 1, 4, 1],
...
def debi_small(out_features, **wargs):
    model = DeBiFormer(
            depth=[2, 2, 9, 3],
...
def debi_base(out_features, **wargs):
    model = DeBiFormer(
            depth=[2, 2, 9, 2],
