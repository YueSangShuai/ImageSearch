...
class MobileOneNet(nn.Module):
    def __init__(self, blocks, ks, channels, strides, width_muls, num_classes, deploy=False):
        ...
        self.feature_info = FeatureInfo([int(channels[1] * width_muls[1]),
            int(channels[2] * width_muls[2]), int(channels[3] * width_muls[3]), 
            int(channels[5] * width_muls[5]), num_classes], 
            [strides[0]*strides[1],
                strides[0]*strides[1]*strides[2], strides[0]*strides[1]*strides[2]*strides[3],
                strides[0]*strides[1]*strides[2]*strides[3]*strides[4]*strides[5], 1024
            ])
    ...
    def forward(self, x, return_features=False):
        ...
        x = self.fc1(x)
        if return_features: return [x1,x2,x3,x5,x]
        return x
    

def make_mobileone_s0(out_features=512, deploy=False):
    blocks = [
        1,2,8,5,5,1
    ]
    strides = [
...
def MobileOneS1(out_features=512, deploy=False):
    blocks = [
        1,2,8,8,5,1
    ]
    strides = [
...
if __name__=="__main__":
    ...
    net = MobileOneS1(out_features=512, deploy=False)
    net.eval()
    y = net(x, return_features=True)
    print(net)
    for x in y: print(list(x.shape))
    torch.save(net.state_dict(), "MobileOneS1_.pth")
    for module in net.modules():
        if hasattr(module, 'switch_to_deploy'):
            module.switch_to_deploy()
    torch.save(net.state_dict(), "MobileOneS1.pth")
