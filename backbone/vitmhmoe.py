...
class MoE(nn.Module):
    ...
    def forward(self, x):
        ...
        gate_scores = gate_scores[:,:,:,None].expand_as(expert_outputs)
        weighted_sum = (expert_outputs * gate_scores).sum(dim=2)
        return weighted_sum


class TokenSplitMerge(nn.Module):
    def __init__(self, num_heads, dim):
        super(TokenSplitMerge, self).__init__()
    ...
    def forward(self, x):
        ...
        assert dim == self.dim
        x = x.view(batch_size, seq_length, self.num_heads, dim // self.num_heads)
        return x.transpose(1, 2).contiguous().view(batch_size * self.num_heads, seq_length, dim // self.num_heads)

    def merge(self, x):
        batch_size_times_heads, seq_length, dim = x.size()
        batch_size = batch_size_times_heads // self.num_heads
        x = x.view(batch_size, self.num_heads, seq_length, dim)
        return x.transpose(1, 2).contiguous().view(batch_size, seq_length, self.dim)

class MHMoE(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_experts, num_heads):
        super(MHMoE, self).__init__()
        self.token_split_merge = TokenSplitMerge(num_heads, input_dim)
        self.moe = MoE(input_dim // num_heads, hidden_dim // num_heads, num_experts)
        self.num_heads = num_heads
        self.input_dim = input_dim

    def forward(self, x):
        x_split = self.token_split_merge(x)
        x_split = self.moe(x_split)
        x_merged = self.token_split_merge.merge(x_split)
        return x_merged
...
class Block(nn.Module):
    def __init__(self,
        ...
        self.mlp = MHMoE(input_dim=dim, hidden_dim=mlp_hidden_dim, num_experts=4, num_heads=num_heads)
...
class L224dp010m000C(VisionTransformer):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(
            img_size=224, patch_size=18, num_classes=out_features, embed_dim=768, depth=24,
            num_heads=8, drop_path_rate=0.10, norm_layer="ln", mask_ratio=0.0, using_checkpoint=True, pos_cnn=True, **kwargs)        

class A224dp050m050(VisionTransformer):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(
            img_size=224, patch_size=28, num_classes=out_features, embed_dim=384, depth=12,
...
class B112e512d24d05m00(VisionTransformer):
    def __init__(self, out_features=512, **kwargs):
        super().__init__(
            img_size=(112, 112), patch_size=(16, 16), num_classes=out_features, embed_dim=512, depth=24,
            num_heads=8, drop_path_rate=0.50, norm_layer="ln", mask_ratio=0.0, using_checkpoint=False, **kwargs)        

if __name__=="__main__":
    net = B112e512d24d05m00()
    net.eval()
