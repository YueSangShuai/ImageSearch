...
class RepVGGplus(nn.Module):
    ...
    def __init__(self, num_blocks, num_classes,
        ...
        self.feature_info = FeatureInfo([int(64 * width_multiplier[0]), int(128*width_multiplier[1]),
            int(256 * width_multiplier[2]), int(512 * width_multiplier[3]), num_classes], [4,8,16,32,1024])
...
def create_RepVGGplus_L2pse(out_features=512, deploy=False, use_checkpoint=False):
    return RepVGGplus(num_blocks=[8, 14, 24, 1], num_classes=out_features,
                  width_multiplier=[2.5, 2.5, 2.5, 5], override_groups_map=None, deploy=deploy, use_post_se=True,
                      use_checkpoint=use_checkpoint)


def RepVGGplusM1(out_features=512, deploy=False, use_checkpoint=False):
    return RepVGGplus(num_blocks=[3, 3, 6, 3], num_classes=out_features,
                  width_multiplier=[0.5, 0.5, 0.5, 1], override_groups_map=None, deploy=deploy, use_post_se=True,
                      use_checkpoint=use_checkpoint)

def RepVGGplusM2(out_features=512, deploy=False, use_checkpoint=False):
    return RepVGGplus(num_blocks=[3, 3, 9, 3], num_classes=out_features,
                  width_multiplier=[0.5, 0.5, 0.5, 1.5], override_groups_map=None, deploy=deploy, use_post_se=True,
                      use_checkpoint=use_checkpoint)

if __name__=="__main__":
    x= torch.rand(1,3,112,112)
    net = RepVGGplusM1(out_features=512)
    net.eval()
    net.switch_repvggplus_to_deploy()
    y = net(x, return_features=True)
    print(net)
    torch.save(net.state_dict(), "RepVGGplusM1.pth")
