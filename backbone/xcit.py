...
class PositionalEncodingFourier(nn.Module):
    ...
    def forward(self, B, H, W):
        ...
        dim_t = self.temperature ** (2 * (dim_t / 2).int() / self.hidden_dim)
...
class XCiT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, out_features=1000, embed_dim=768,
        ...
        self.feature_info  = FeatureInfo([self.num_features, num_classes], [1024, 1024])
    ...
    def forward(self, x, return_features=False):
        ...
        x = self.head(x0)
        if return_features: return x0, x
        return x
...
if __name__=="__main__":
    ...
    net = xcit_tiny_12_p8(True, out_features=512)  # xcit_tiny_12_p8_384_dist(out_features=2)
    net.eval()
    y = net(x)
    print(net)
    print(y.shape)
    torch.save(net.state_dict(), "xcit_tiny_12_p8.pth")
