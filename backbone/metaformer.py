...
from .utils import FeatureInfo, to_2tuple, trunc_normal_, DropPath
...
class MetaFormer(nn.Module):
    ...
    """
    def __init__(self, channel=3, num_classes=1000, 
                 depths=[2, 2, 6, 2],
                 dims=[64, 128, 320, 512],
        ...
        self.downsample_layers = nn.ModuleList(
            [downsample_layers[0](channel, dims[0])] + \
        ...
        self.feature_info = FeatureInfo(dims+[num_classes], [4, 8, 16, 32, 1024])
    ...
    def forward_features(self, x):
        ...
        return outs, self.norm(x.mean([1, 2])) # (B, H, W, C) -> (B, C)
...
class identityformer_s12(MetaFormer):
    def __init__(self, out_features, **kwargs):
        super().__init__(num_classes=out_features,
            depths=[2, 2, 6, 2],
            dims=[64, 128, 320, 512],
            token_mixers=nn.Identity,
...
class identityformer_s24(MetaFormer):
    def __init__(self, out_features, **kwargs):
        super().__init__(num_classes=out_features,
            depths=[4, 4, 12, 4],
            dims=[64, 128, 320, 512],
...
class convformer_s12(MetaFormer):
    def __init__(self, out_features, **kwargs):
        super().__init__(num_classes=out_features,
            depths=[2, 2, 6, 2],
            dims=[64, 128, 320, 512],
...
class convformer_s18h(MetaFormer):
    def __init__(self, out_features, **kwargs):
        super().__init__(num_classes=out_features,
            depths=[3, 3, 9, 3],
            dims=[32, 64, 160, 256],
...
class convformer_s18q(MetaFormer):
    def __init__(self, out_features, **kwargs):
        super().__init__(num_classes=out_features,
            depths=[3, 3, 9, 3],
            dims=[48, 80, 192, 384],
...
class caformer_s18h(MetaFormer):
    def __init__(self, out_features, **kwargs):
        super().__init__(num_classes=out_features,
            depths=[3, 3, 9, 3],
            dims=[32, 64, 160, 256],
...
if __name__=="__main__":
    import sys
    from . import utils
    model = utils.test_model(eval(sys.argv[1]), [3,112,112])
    torch.save(model.state_dict(), f"ftr100model/{model.__class__.__name__}.pth")