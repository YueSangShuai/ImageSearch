...
def get_args():
    ...
    parser.add_argument("--data", type=str, default="d.jsonl")
    parser.add_argument("--text-backbone", type=str, default="tbackbone.minimind.L1")
    parser.add_argument("--teachor", default="mexma-siglip2")
    ...
    # loss weights
    parser.add_argument('--teachor-info-weight', type=float, default=0.1, help='权重系数: InfoNCE loss')
    parser.add_argument('--temperature', type=float, default=0.05, help='Temperature for InfoNCE loss')
    parser.add_argument('--teachor-rel-weight', type=float, default=1.0, help='权重系数: 师生关系损失')
    parser.add_argument('--teachor-sim-weight', type=float, default=10.0, help='权重系数: 自相似度损失')
    parser.add_argument('--teachor-l2-weight', type=float, default=0, help='权重系数: mse loss')
    parser.add_argument('--teachor-l1-weight', type=float, default=1, help='权重系数: smooth l1 loss')
    ...
    if is_main_process(): 
        for temp in range(10000):  # 找到一个新目录存放本次实验日志和模型数据
            exp_name = '%s%s-%s-%s-%d/%d' % (args.name, args.arch, args.text_backbone, 
