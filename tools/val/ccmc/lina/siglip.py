...
class ImageTextEmbedder:
    ...
    def add_arguments(parser):
        if "--model-file" not in parser._option_string_actions:
            parser.add_argument("--model-file", type=str, default="/data/yuesang/LLM/VectorIE/models/mexma-siglip2", help="Path to the model file.")
        if "--use-cuda" not in parser._option_string_actions:
            parser.add_argument("--use-cuda", action='store_true',
                                help="Use CUDA for inference.")
    def __init__(self, 
        model_file: str="/data/yuesang/LLM/VectorIE/models/mexma-siglip2", 
        use_cuda: bool=True,
        **kargs):
        device = "cuda"
