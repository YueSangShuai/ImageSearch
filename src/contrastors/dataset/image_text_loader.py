...
class ImageTextDataset(Dataset):
    def __init__(self, list_file, classifer_config,transform=None, tokenizer=None, add_eos=False, add_prefix=False,teacher_transformer=None):
    ...
    def _load_samples(self):
        ...
        with open(self.list_file, "r", encoding="utf-8") as f:
            try:
                ...
                raise ValueError(f"JSON 文件解析失败，错误信息: {e}")

        for sample in data:
            img_path = sample.get("image_path")
            txt_path = sample.get("text_path")
            label_dict = {}
            
            # 检查必要字段是否存在
            if not img_path or not txt_path:
                print(f"警告：样本缺少 'image_path' 或 'text_path'，跳过")
                continue
            
            for cls_name in self.classifer_config.classes:
                # 从样本中获取当前类别的标签值
                label = sample.get(cls_name)
                
                if label is None:
                    # 标签缺失，设置默认值（如 -1，表示无标签）
                    print(f"警告：样本缺少类别 '{cls_name}' 的标签，使用默认值 -1")
                    label_dict[cls_name] = -1
                else:
                    # 确保标签是整数（根据需求调整类型）
                    try:
                        label_dict[cls_name] = int(label)
                    except (ValueError, TypeError):
                        print(f"警告：类别 '{cls_name}' 的标签无法转换为整数（值为 {label}），使用默认值 -1")
                        label_dict[cls_name] = -1
            
            if any(v == -1 for v in label_dict.values()):
                invalid_samples.append((img_path, txt_path, {k: v for k, v in label_dict.items() if v == -1}))
            
            
            # 添加样本到列表（无论标签是否完整）
            samples.append((img_path, txt_path, label_dict))
    ...
    def __getitem__(self, idx):
        img_path, txt_path,label_dict = self.samples[idx]
        orign_image = Image.open(img_path).convert("RGB")
        
        if self.teacher_transformer is not None:
            teacher_image=self.teacher_transformer(images=[orign_image], return_tensors="pt")
        
        if self.transform:
            image = self.transform(orign_image)

        with open(txt_path, "r", encoding="utf-8") as f:
            text = f.read().strip()

        tokenized = tokenize(text, self.tokenizer, self.add_eos, self.add_prefix)
...
def get_local_image_text_dataset(args, classifer_config,transforms, is_train, tokenizer, epoch=0, add_eos=False, add_prefix=False,teacher_transformer=None):
    
    root_dir = args.image_text_shards if is_train else args.imagenet_val_path
    dataset = ImageTextDataset(
        list_file=root_dir,
        classifer_config=classifer_config,
