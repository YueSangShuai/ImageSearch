...
import timm
from  contrastors.models.emov2 import *
import timm
...
class BiEncoder(PreTrainedModel):
    def __init__(self, config):
        ...
        if "EMO" in config.model_name:
            self.trunk=MODEL.get_module(config.model_name)(pretrained=False, num_classes=1000)
            # pretrained_dict = torch.load("/data/yuesang/LLM/contrastors/emov2_pre/emov2_pretrained_weights/EMOv2_2M_224.pth")
            # model_dict = self.trunk.state_dict()
            # # 只保留预训练权重中与模型结构匹配的键
            ...
            # self.trunk.load_state_dict(model_dict)
            # print(self.trunk)
            ...
            if getattr(config, "nomic_encoder", True) is False:
                ...
                if "dinov2" in config.model_name:
                    ...
                    if config.pretrained:
                        if "nomic-embed-vision-v1.5" in config.model_name :
                            self.trunk=AutoModel.from_pretrained(
                                config.model_name, config=model_config, trust_remote_code=True
                                )
                        elif "mobilevit" in config.model_name:
                            from transformers import MobileViTModel

                            model_config = AutoConfig.from_pretrained(config.model_name, trust_remote_code=True)
                            if config.pretrained:
                                self.trunk = MobileViTModel.from_pretrained(config.model_name, config=model_config)
                            else:
                                self.trunk = MobileViTModel(config=model_config)
                        else:
                            self.trunk = AutoModel(config=model_config)
                    else:
                        self.trunk = DecoderModel(config=model_config)

                    if not "mobilevit" in config.model_name:
        ...
        if config.projection_dim:
            if "mobilevit" in config.model_name:
                self.proj = nn.Linear(320, config.projection_dim)
            elif "EMO" in config.model_name:
                self.proj = nn.Linear(self.trunk.output_dims, config.projection_dim)
            else:
    ...
    def forward(self, input_ids, attention_mask=None, is_padded_inputs=True, normalize=True, binarize=False, **kwargs):
        ...
        with context():
            if "mobilevit" in self.config.model_name:
                input_ids = input_ids.to(dtype=self.trunk.dtype, device=self.trunk.device)
                trunk_output = self.trunk(input_ids, **kwargs)
            elif "EMO" in self.config.model_name:
                input_ids = input_ids.to(dtype=torch.bfloat16)
                trunk_output = self.trunk(input_ids)
            else:
        ...
        if "EMO" in self.config.model_name:
            trunk_output=trunk_output['out']
        else:
        ...
        if "mobilevit" in self.config.model_name:
            embedding = embedding.mean(dim=[2, 3])  # 简洁，等价于上面一行
